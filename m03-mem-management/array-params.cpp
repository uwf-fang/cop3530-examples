...
void matrixEleMultiply3(double matrix[ROWS][COLS], int rows, int cols,
                        double factor);
void matrixEleMultiply4(double matrix[][COLS], int rows, int cols,
                        double factor);
void ThreeDimEleMultiply5(double ThreeDimArray[][5][10], int rows, int cols,
                          double factor);
...
int main(int argc, char const* argv[]) {
  ...
  for (int i = 0; i < ROWS; ++i)
    for (int j = 0; j < COLS; ++j) dynMat[i][j] = i * COLS + j + 1;  // 1 to 10

  DynMatrixEleMultiply(dynMat, ROWS, COLS, 2);
  printMatrix(dynMat, ROWS, COLS);

  for (int i = 0; i < ROWS; ++i) delete[] dynMat[i];
  delete[] dynMat;

  return 0;
}

void matrixEleMultiply1(double matrix[2][5], int rows, int cols,
                        double factor) {
  for (int i = 0; i < rows; ++i)
    for (int j = 0; j < cols; ++j) matrix[i][j] *= factor;
}
